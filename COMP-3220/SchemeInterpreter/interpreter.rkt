       ...
       (cond
        ...
        ((eqv? (car stmt) 'if)
         (if (exp myEnv (cadr stmt))
             (if (and (pair? (caddr stmt)) (eqv? (car (caddr stmt)) 'then))
               (process (cadr (caddr stmt)) myEnv)
               (report-invalid-env stmt))
           myEnv))
        ((eqv? (car stmt) 'while)
         (let loop ((env myEnv))
         (if (exp env (cadr stmt))
             (if (and (pair? (caddr stmt)) (eqv? (car (caddr stmt)) 'then))
                 (loop (process (cadr (caddr stmt)) env))
                 (report-invalid-env stmt))
             env)))
...
(define exp
  (lambda (myEnv e)
    (cond
      ...
      ((eqv? (car e) 'if) (if (exp myEnv (cadr e))(exp myEnv (caddr e))(exp myEnv (cadddr e))))
