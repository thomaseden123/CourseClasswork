...
public class myFirstUDPClient {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.out.println("Usage: java myFirstUDPClient <Server> <Port>");
            return;
        }
        ...
        try (DatagramSocket sock = new DatagramSocket();
            ...
            while (count < 7) {
                ...
                try {
                    ...
                    long roundTripTime = (System.nanoTime() - startTime) / 1000000;
                    int read = receivedPacket.getLength();
                    if (read < 4) {
                        System.out.println("Invalid response from server.");
                        continue;
                    } else {
                        String decoded = new String(receivedPacket.getData(), 2, read - 2, "UTF-16").trim();
